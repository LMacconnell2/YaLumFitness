<script lang="ts">
  ...
  async function generateMealPlan() {
    ...
    isGenerating = true;
    try {
      const response = await fetch("http://localhost:3000/api/v1/ai/generate-meals", {
        method: "POST",
      ...
      if (response.ok) {
        alert("Nutrition plan generated!");
...
<div class="modal-backdrop" onclick={() => showModal = false} aria-hidden="true">
  <div class="modal-content" onclick={e => e.stopPropagation()} aria-hidden="true">
    ...
    <p>Which assessment should Gemini use to calculate your macros?</p>
